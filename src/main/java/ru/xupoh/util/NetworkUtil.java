...
public class NetworkUtil {
	...
	public static ArrayList<Object> decodeData(Class<?>[] types, ByteBuf buffer) throws IOException {
		...
		for (Class<?> clazz : types) {
			assert clazz != null : "Null class can't be decoded!";
	...
	static {
		...
		writers.put(String.class, (obj, buffer) -> {
			writeUTF8String(buffer, (String) obj);
		});
		...
		writers.put(Collection.class, (obj, buffer) -> {
			try {
				encodeData(buffer, (Collection<Object>) obj);
			} catch (Exception e) {
				e.printStackTrace();
			}
		});
	...
	public static String readUTF8String(ByteBuf from) {
		...
		String str = from.toString(from.readerIndex(), len, Charset.forName("UTF-8"));
	...
	public static void writeUTF8String(ByteBuf to, String string) {
		byte[] utf8Bytes = string.getBytes(Charset.forName("UTF-8"));
